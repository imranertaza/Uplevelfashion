	...
	$.fn.pavoSliderEditor = function( initvar ) {
		...
		this.createList = function( JSLIST  ){
 		 ...
 		 * Process All First Handler.
  		 */
		this.process = function( siteURL , adminURL, delayTime, imageURL ) {
		...
		this.preview=function(){
			...
			$.ajax({
				url: 'index.php?route=extension/module/pavsliderlayer/previewLayer&user_token=' + getURLVar('user_token'),
				type: "POST",
		...
		this.insertTypo=function(){
			...
			$('#modal-typo').remove();
			$.ajax({
				url: 'index.php?route=extension/module/pavsliderlayer/typo&user_token=' + getURLVar('user_token'),
				type: "POST",
		...
		this.showDialogImage=function( thumb ){ /// alert( thumb );
			$('#modal-image').remove();
			$.ajax({
				url: 'index.php?route=extension/module/pavsliderlayer/filemanager&thumb='+thumb+'&user_token=' + getURLVar('user_token'),
				dataType: 'html',
				success: function(html) {
		...
		this.createDefaultLayerData = function( layer, data ){
	 		...
	 		if( data !=null && data ) {
		 		...
		 		if(  data['layer_type'] == 'image' ){
					...
					var src = this.imageURL+data['layer_content'];
