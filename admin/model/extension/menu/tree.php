...
class ModelExtensionMenuTree extends Model {		
	...
	public function getTree( $parent=1 , $edit=false, $params, $store_id = 0 ){
		...
		foreach($childs as $child ){
			$child['megaconfig'] = $this->hasVerticalMenuConfig( $child );
		...
		if( $this->hasChild($parent) ){
			...
			foreach( $data as $menu ){
				...
				if( $this->hasChild($menu['verticalmenu_id']) || $menu['type_submenu'] == 'html'){
					...
					$output .= $this->genVerticalMenuByConfig( $menu['verticalmenu_id'], 1, $menu );
				}else {
	 ...
	 *
	 */
	public function genVerticalMenuByConfig( $parentId, $level,$menu  ){
	...
	public function hasVerticalMenuConfig( $menu ){
	...
	public function renderAttrs( $menu ){  
		$t = sprintf( $this->_editString, $menu['verticalmenu_id'], $menu['is_group'], $menu['colums']  );
		if( $this->_isLiveEdit  ){  
			if( isset($menu['megaconfig']->subwidth) &&  $menu['megaconfig']->subwidth ){
				$t .= ' data-subwidth="'.$menu['megaconfig']->subwidth.'" ';
			}
			$t .= ' data-submenu="'.(isset($menu['megaconfig']->submenu)?$menu['megaconfig']->submenu:$this->hasChild($menu['verticalmenu_id'])).'"'; 
	...
	public function renderMenuContent( $menu , $level ){
		...
		if( $this->hasChild($menu['verticalmenu_id']) ){
			...
			$output .= $this->genVerticalMenuByConfig( $menu['verticalmenu_id'], $level, $menu );
		}else {
			$output .= '<li class="'.$menu['menu_class'].'" '.$this->renderAttrs($menu).'>';
			if( $menu['show_title'] ){ 
