...
class ControllerExtensionModulePavobuilder extends Controller {
	...
	private function form( $id = 0 ) {
		...
		$this->data['iso_code'] = isset($this->session->data['language']) ? $this->session->data['language'] : false;
		if (!$this->data['iso_code']) {
			$languages = $this->model_localisation_language->getLanguages();
			$language = current($languages);
			$this->data['iso_code'] = $language['code'];
		}
...
//			$this->document->addScript( '//maps.googleapis.com/maps/api/js?key=' . $this->config->get( 'pavothemer _google_map_api_key' ) . '&libraries=places' );
	...
	public function buttons(){
		$this->load->language( 'extension/module/pavobuilder' );
		$this->load->model( 'setting/module' );
		$this->data['blockbuilders'] = $this->model_setting_module->getModulesByCode( 'pavobuilder' );
		$this->data['options']  = array(
			0 => $this->language->get( 'text_no' ),
			1 => $this->language->get( 'text_yes' )
		);
		echo  $this->load->view( 'extension/module/pavobuilder/buttons', $this->data );die;

	}

	public function install() {
		$this->load->model('extension/pavobuilder/pavobuilder');
		$this->model_extension_pavobuilder_pavobuilder->intsall();
	}
