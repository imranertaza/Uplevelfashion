 ...
 * @version 1.0
 */
class ControllerExtensionModulePavlayerednavigation extends Controller {

	/**
	 * Load javavscript lib files
	 */
	public function loadScript(){
		$this->document->addScript('catalog/view/javascript/jquery/ion.rangeSlider.min.js' );
		$this->document->addScript('catalog/view/javascript/jquery/pavlayeredfilter.js' );
		$this->document->addStyle('catalog/view/javascript/jquery/ion.rangeSlider.css' );
	}
	...
	public function index() {
		...
		if ( $is_ajax ) {
			...
			if ($category_info) {
				...
				$this->load->language('extension/module/pavlayerednavigation');
				...
				$data['store_action'] = str_replace('&amp;', '&', $this->url->link('product/category', 'path=' . $this->request->get['path'] . $url));
				$data['action'] = str_replace('&amp;', '&', $this->url->link('extension/module/pavlayerednavigation/filter', 'ajax=1&path=' . $this->request->get['path'] . $url));
				...
				$this->load->model('extension/module/pavlayerednavigation');
				$fprice = $this->model_extension_module_pavlayerednavigation->getMinPrice( $filter_data );
				...
				foreach( $manufacturers as $key => $manufacturer ){
					...
					$manufacturer['filter_name'] = $manufacturer['name'];
					$manufacturer['name'] .= ' ('.$this->model_catalog_product->getTotalProducts($filter_data).')';
				...
				if ($filter_groups) {
					...
					return $this->load->view( 'extension/module/pavlayerednavigation', $data );
				}
			} else {
				$url = '';
				if (isset($this->request->get['path'])) {
	...
	/**
	 * render content ajax to show products
	 */
	public function filter(){
		$products = $this->getContentRender();
		$data = array(
			'products' 		=> $products,
			'status'		=> true,
	...
	public function getContentRender() {
		...
		if ($category_info) {
			...
			return $this->load->view( 'extension/module/pavlayeredcategory', $data );
		} else {
			$url = '';
			if (isset($this->request->get['path'])) {
				$url .= '&path=' . $this->request->get['path'];
